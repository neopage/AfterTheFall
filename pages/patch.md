...
- [2024-04-04　パッチ 1.9](./patch.md#2024-04-04%E3%83%91%E3%83%83%E3%83%81-19)
- [2023-02-22　パッチ 1.8](./patch.md#2023-02-22%E3%83%91%E3%83%83%E3%83%81-18)
- [2022-10-04　Hotfix](./patch.md#2022-10-04hotfix)
- [2022-09-15　パッチ 1.7](./patch.md#2022-09-15%E3%83%91%E3%83%83%E3%83%81-17)

## 2024-04-04　パッチ 1.9

[出典：Patch 1.9 – ナイトフォール アップデート](https://www.afterthefall-vr.com/patch-1-9-nightfall-update/)

### ホードマップ追加
- ハイライズ: この超高層ビルの頂上
- 倉庫: 武器の隠し場所や木箱がいっぱいの廃倉庫
- マーケット: チャイナタウンのライトアップされた通り

### 武器追加: ハンティングライフル
- ハンティングライフルには氷の装甲を貫通する徹甲弾があり、高難易度でも狙いを定めた一撃で複数のスノーブリードを倒すことが可能
- リボルバーにも使われるグリーン弾を使用
- アップグレードされたバレルは、弾丸貫通力を 0 から +1 増加させる
- 合計 18 個のアタッチメントと 37 個の新しいスキン
- プロトタイプフロッピー（赤）を復号化することで購入可

### ユーザーインターフェース
- ロビーブラウザが追加され、利用可能なすべての試合をすばやく確認できる
- 招待プレビューが追加され、招待を承諾する前に、どのモード、難易度、レベルに招待されたかを確認できる
- 最近のプレイヤー画面が追加され、最近対戦したプレイヤーのプロフィールを開くことができるようになった
- Horde中に表示されるスコア画面を追加。メニューを開かなくても、キル数や到達したウェーブ数を確認できる
- すべての UI インタラクションに触覚フィードバックを追加し、メニューの操作が簡単になった
- 終了画面の UI を改良し、フロッピーの収集と復号化がより楽しく

### ゲーム体験の改善
- フロッピーはセーフルームに正常に保管できた場合、死亡しても入手可
- フロッピーのビューが改善され、実行中にフロッピーを見つけやすくなった
- ランを正常に完了すると、フロッピー報酬が２倍に
- スノーブリードにダメージを与えた際に得られる収穫が 50% 増加
- 重複フロッピーから得られる収穫は、難易度ごとに増加しなくなった
- レベル内に出現するフロッピー ディスクの数が 2 から 3 に増加

### 修正、変更、調整
- 武器庫からゲーム開始するときに武器がストックにリセットされる問題を修正
- Quest3でマイクが聞き取りにくい、またはまったく聞こえない問題を修正
- Horde でフリーズした場合、5wave後ではなく 1wave後に復活
- ミューテーターは、以前の8時間ではなく、2時間で循環
- PCプレイヤー向けに、保持アイテムのオブジェクトとワールドの衝突をオフにするオプションを追加
- プライベートとパブリックの両方のハーベスト ランで AI なしでプレイするオプションを追加しました
- 4つのユニークな新しいメカニック スキンを追加しました。ハンティングライフルを使用してロック解除可
- アリゾナ ファルコン、ポンプ ショットガン、サブマシンガン、アサルト カービン、MT-47、リボルバーにガス キャニスター アタッチメントを追加しました。弾丸の速度が大幅に上がり、アイスアーマーに 15% のダメージ
- ショットガンマガジンを追加しました。これにより、最大弾薬容量が増加し、Advanced Reload でのショットガンのリロードが容易に
- ほぼすべての重要な項目にアウトラインを追加し、環境から区別しやすく
- ハブ内に一定数のプレイヤーがいる場合にゲームが適切にシャットダウンしないバグを修正
- すべてのプレイヤーが存在する場合、コンパニオン AI が自動的にセーフルームにテレポートされるという防御修正を追加
- 最終エリアをアクティブにするには、人間のプレイヤーのみが存在する必要があり
- すべてのホードモードマップに出現するスペシャルのリストに、ガンナーとチャージャーを追加
- ガンナーとチャージャーのスペシャルの体力とスタガー体力を増加
- バッテリーパックのクリティカル確率を 10% から 15% に増加
- まだロックを解除していない武器スキンが表示されない問題を修正
- すべてのロックを解除する前にチップセットが重複して取得される可能性があるバグを修正
- 収穫UI に、収穫量が多い場合に間違った数値が表示されなくなった
- PlaystationVR で手榴弾を投げるのがはるかに簡単になった

## 2023-02-22　パッチ 1.8 

[出典：Patch 1.8 – Descension Update](https://www.afterthefall-vr.com/patch-1-8-descension-update/)

### PSVR2に対応
- PSVR1用のATF デジタルを所有している場合、PSVR2でも無料で利用可能
...
### クイックプレイ追加(収穫作戦)
- すべてのマップをクリアしているレベル以下のチームに追加される

### バグ修正
- ホードモードでウェーブ30 ～ 40以上を過ぎるとスタックする問題を修正
- 無限ショットガンを修正(Frozen Pool チップセットが砲弾ではなく各ペレットに適用)
- チップセットのインスタキルがスコア画面で異常に高いダメージとして表示される問題を修正
- Frozen Pool チップセットの銃を手動リロードしたときに弾が減らないバグを修正
- ホストが試合から退出したときに手動グリップモードを使用するとプレイヤーが武器を失う問題を修正
- 武器庫に関連するさまざまな問題を修正
- 武器の射撃が停止しても弾薬カウンターが減り続ける問題を修正
- ドーナツスキンが適切にアンロックされない問題を修正
- ネオンイエローのスキンがアンロックされない問題を修正
- 弾薬ベルトを調整した後にリストデバイスが無効になる問題を修正
- ショットガンの視覚的な不具合を修正(ショットガンが部分的にプレイヤーの手の内側に見える)
- チップセットが返金できなくなった
- プレイヤーが閉じ込められたりするのを防ぐために、ダウンタウンのセーフルームを調整
- ゲームがフェイススキャナーでスタックするバグを修正
- 頭を振ると衝撃波装置が発射される問題を修正
- ダウンタウンのジップラインでプレイヤーが床に閉じ込められる問題を修正
- スタンドに武器がない場合、チップセットを購入できない問題を修正
- リレータワーの開始時にフロッピーディスクが略奪できないように修正
- レーザー照準器の視覚的な調整
- ダウンタウンレベルのいくつかのアート修正

### バランス調整
- ショットガン反動を減少
- トミーガンの反動を戻した
- ポンプショットガンの自動スライド時間を以前のものに調整
- すべてのLast Standのダメージ倍率を下げた
